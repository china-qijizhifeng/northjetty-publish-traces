...
2. For trace-backed builds, inspect only the supplied paths. Prefer `.trace.json.gz`; raw `.trace.json` also works.
   The builder records a timestamp for every trace, preferring a 10-digit Unix timestamp in the filename and falling back to the source file mtime. The viewer exposes a searchable, date-grouped scene combobox plus compact rank and trace selectors, sorts newest first, and persists valid `scene`, `rank`, and `trace` selections in the URL.
3. Build the site:
